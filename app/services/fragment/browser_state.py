...
async def collect_fragment_page_state(page: Any) -> dict[str, Any]:
    ...
    disconnect_ton_visible = await page.get_by_text("Disconnect TON", exact=True).count() > 0
    connect_cta_visible = connect_wallet_visible or connect_ton_visible

    ton_connect_keys = [key for key in local_storage_keys if str(key).startswith("ton-connect")]
    wallet_session_ready = (bool(ton_connect_keys) and not connect_cta_visible) or disconnect_ton_visible
    ...
    return {
        ...
        "fragment_disconnect_ton_visible": disconnect_ton_visible,
