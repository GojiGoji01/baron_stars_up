...
async def handle_profile_withdraw_stars(callback: CallbackQuery) -> None:
    await safe_answer_callback(callback, PROFILE_WITHDRAW_STARS_ALERT, show_alert=True)
...
async def handle_referrals_withdraw(callback: CallbackQuery) -> None:
    await safe_answer_callback(callback, REFERRAL_WITHDRAW_ALERT, show_alert=True)
